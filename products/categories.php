...
requirePermission('products.manage'); // catalog management — owner only per role matrix
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!csrfValidate($_POST['csrf_token'] ?? null)) {
        ...
        if ($action === 'create' || $action === 'update') {
            ...
            } elseif (CategoryService::nameExists($name, $editId)) {
                $errors['name'] = 'A category with this name already exists.';
            } else {
                $data = ['name' => $name, 'description' => $description, 'status' => $status];
                if ($action === 'create') {
                    CategoryService::create($data, (int) $_SESSION['user_id']);
                    $flash = ['type' => 'success', 'text' => 'Category added.'];
                } else {
                    CategoryService::update($editId, $data, (int) $_SESSION['user_id']);
                    $flash = ['type' => 'success', 'text' => 'Category updated.'];
                }
            }
        }
    }
...
<div class="page-header d-flex justify-content-between align-items-center flex-wrap gap-2">
    <div>
        <nav class="breadcrumb"><a href="<?= BASE_URL ?>/products/index.php" class="text-decoration-none">Products</a>&nbsp;/&nbsp;Categories</nav>
        <h1 class="page-title h4 mb-0">Categories</h1>
    </div>
    <button class="btn btn-primary btn-sm" data-bs-toggle="modal" data-bs-target="#categoryModal" onclick="openCategoryModal()"><i class="bi bi-plus-lg me-1"></i>Add Category</button>
</div>
...
<div class="ts-card">
    <?php if (!$categories): ?>
        <div class="empty-state"><i class="bi bi-tags"></i><p class="mb-0">No categories yet. Add your first one.</p></div>
    <?php else: ?>
            ...
            <table class="table align-middle mb-0">
                ...
                <?php foreach ($categories as $c): ?>
                    <tr>
                        ...
                        <td><span class="badge-status <?= $c['status'] === 'active' ? 'badge-success' : 'badge-neutral' ?>"><?= ucfirst($c['status']) ?></span></td>
                        <td class="text-end">
                            <button class="btn btn-sm btn-outline-secondary"
                                    onclick='openCategoryModal(<?= json_encode($c, JSON_HEX_APOS | JSON_HEX_QUOT) ?>)'
...
function openCategoryModal(cat) {
    ...
    document.getElementById('cat_action').value = cat ? 'update' : 'create';
    document.getElementById('cat_id').value = cat ? cat.id : '';
    document.getElementById('cat_name').value = cat ? cat.name : '';
    document.getElementById('cat_description').value = cat ? (cat.description || '') : '';
    document.getElementById('cat_status').value = cat ? cat.status : 'active';
}
</script>
